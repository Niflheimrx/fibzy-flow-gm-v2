...
PrintRainbowText(10, "RNGFix: Client-Sided Loaded")
...
--[[RNGFix.g_iTick = g_iTick
	...
	RNGFix.Telehop = CreateConVar("rngfix_telefix", "0", FCVAR_NOTIFY, "Enable TeleFix", 0, 1):GetBool() -- Might not be necessary for every map (and it can cause issues with some maps), so it's disabled by default
	...
	SERVER_GRAVITY = GetConVar "sv_gravity":GetInt()
